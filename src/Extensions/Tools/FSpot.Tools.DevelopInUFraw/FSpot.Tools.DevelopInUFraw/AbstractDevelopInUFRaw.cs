	...
	public abstract class AbstractDevelopInUFRaw : ICommand
	{
		...
		protected void DevelopPhoto (Photo p)
		{
			...
			case "ufraw-batch":
				...
				if (File.Exists (Path.Combine (Configuration.BaseDirectory, "batch.ufraw"))) {
					// We found an ID file, use that instead of the raw file
					idfile = "--conf=" + GLib.Shell.Quote (Path.Combine (Configuration.BaseDirectory, "batch.ufraw"));
				}
				break;
			}
			...
			if (File.Exists (Path.ChangeExtension (developed.ToString (), ".ufraw"))) {
				...
				File.Delete (Path.Combine (Configuration.BaseDirectory, "batch.ufraw"));
				File.Copy (Path.ChangeExtension (developed.LocalPath, ".ufraw"), Path.Combine (Configuration.BaseDirectory, "batch.ufraw"));
